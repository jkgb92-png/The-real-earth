...
exports.WorkerTileCache = void 0;
// ── Coordinate helpers ────────────────────────────────────────────────────────
...
function latToTileY(lat, z) {
    ...
    return Math.floor(((1 - Math.log(Math.tan(latRad) + 1 / Math.cos(latRad)) / Math.PI) / 2) *
        2 ** z);
}
// ── Class ─────────────────────────────────────────────────────────────────────
class WorkerTileCache {
    constructor() {
        this.worker = null;
        this.seen = new Set();
     ...
     */
    prefetchViewport(template, zoom, north, south, east, west, padding = 1, maxTiles = 24) {
        if (!this.worker)
            return;
...
exports.WorkerTileCache = WorkerTileCache;
